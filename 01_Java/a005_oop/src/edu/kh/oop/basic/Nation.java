...
public class Nation {
	...
	// 국민이라면 가지고 있어야 하는 공통된 속성(정보, 데이터)
	// -> 추상화(공통점만 추출, 불필요한 요소는 제거)
	
	/* 속성(== 멤버 변수)*/
	private String pNo; // 주민등록번호 (모두다 숫자여야 숫자형)
	// [캡슐화] 두 번째 특징(원칙)
	// 멤버변수는 private으로 지정
	...
	public void speakKorean() {
		System.out.println("가나다라 한국말 가능(세종대왕 만세)");
	}
	...
	// [캡슐화] 두 번째 특징(원칙)
	// private으로 지정된 멤버변수에 대한 
	// 간접 접근 방법(기능)을 작성
	public String getpNo() { // 외부에서 내부 대이터를 얻어가는 방법
		return this.pNo;
	}
