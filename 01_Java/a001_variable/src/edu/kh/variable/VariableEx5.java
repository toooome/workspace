...
public class VariableEx5 {
	...
	public static void main(String[] args) {
		...
		// 연산 결과를 예측해서 저장할 변수의 자료형을 선택 
		// 예) 아래의 자료형을 int로 적으면 오류가 남
		...
		System.out.println(ch5 + "의 Unicode Number : " + num5);
		System.out.println("--------------------");
		
		/* [강제 형변환]
		 * - 값의 범위가 큰 자료형을 작은 자료형으로 강제 변환 (대부분)
		 * 
		...
		byte test2 = (byte)test1; // Type mismatch: cannot convert from int to byte
					// 변환 시키고자 하는 자료형을 ()안에 명시 
		
		System.out.println("test1 : " + test1);
		System.out.println("test2 : " + test2);

		// ctrl + art + 방향키 위 + 아래 : 라인 복사 
		// 화면 뒤집힐 경우!
		...
		// bNum = bnum + 1; // 자료형이 byte = byte + int로 다름
		bNum = (byte)(bNum + 1);
		//     (byte)((int)127)
		...
		// int iNum3 = iNum1 + inum2; // 30억이 나와야 하지만 
		
		// 해결 방법
		long iNum3 = (long)iNum1 + iNum2; // 30억
		
		System.out.println(iNum3);
		
		// 값의 범위를 잘못 예측한 경우 오버플로우가 발생함!!
		// 예) 유튜브 강남스타일 조회수 마이너스 표기 (오버플로우 사례)
