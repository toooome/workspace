...
//		String[] menuArr = new String[10]; // 메뉴 개수가 한정됨
	...
	public void ex7() {
		...
		int [] arr = {100,2000,300,400,500,600,700,800,1000};
...
//		}
//		
//		if(index < 0 ) { // 일치하지 않는 결과가 없음
//			System.out.println("존재하지 않습s니다.");
//				
//			} else {
...
//			}
		...
		// 검색 for문 종료 후 flag가 true : 검색 결과 없음
		//					 		false : 검색 결과 존재
		...
		for(int i=0 ; i<arr.length ; i++ ) {
			...
			if(input == arr[i]) { // 입력값과 일치하는 값이 존재하는 경우
			   index = i;
			   flag = false;
			   break; // 일치하는 값을 찾은 경우 더이상 반복하지 않음 (성능 개선)
			}
	...
	public void ex8() {
		...
		// 서로 다른 참조변수가 하나의 배열, 객체를 참조 (공유 개념)
		
		// 깊은 복사
		// - 원본과 같은 자료형, 크기는 같거나 더 큰 배열을 만듦어 
		//	 원본의 데이터를 모두 복사하는 방법 (복제 개념)
		...
		System.out.println("copyArr : " + copyArr1);
		
		// arr1 : [I@6f2b958e
		// copyArr : [I@6f2b958e [같음]
...
//		}
		...
		// [참고] System.arraycopy()를 이용한 깊은 복사
		// shift + f2
		...
		System.out.println("copyArr2 : " + Arrays.toString(arr2));
